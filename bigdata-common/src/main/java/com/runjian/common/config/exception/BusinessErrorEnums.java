...
public enum BusinessErrorEnums {
    ...
    USER_TOKEN_IS_GONE(401, 20001, "token找不到"),
    ...
    public static BusinessErrorEnums getByCode(Integer errCode) {
        if (errCode == null) {
            return null;
        }
        for (BusinessErrorEnums businessErrorEnums : BusinessErrorEnums.values()) {
            if (errCode.equals(businessErrorEnums.getErrCode())){
